...
One is absolutely centred, which takes it out of the flex flow so `space-between` pins Neotone and neOS to the two boundaries. The gaps are deliberately unequal — the wordmarks are different widths, so all three constraints cannot hold with even spacing. Alignment to the page grid wins over even rhythm.
...
Page bodies are six placeholder blocks. They are not yet click targets and most do not yet have a reader.
...
The landing page used to be special-cased — all four marks rendered full size — which required an `inactiveScale` exception, a `targetScale` exception, a separate `ACTIVE === 0` entry branch, and a `prevIsLanding` flag carried through `sessionStorage`. Removing the exception removed all four.
...
2. **Tonefield merch** (`EUR 12`) has no home — Tonefield is an article now. Drop, or re-home later?
...
v2/
  README.md         this file
  index.html        Neotone — container, root
